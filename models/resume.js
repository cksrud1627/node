  ...
  */
  update : async function(params) {
    ...
    try {
      ...
      /** 취업우대 병역 선택 체크 S */
     if (!params.items || params.items.indexOf("병역") == -1){ // 취업우대,병역 항목선택이 없으면 값 비우기
      ...
      if (params.items && params.items.indexOf('학력') != -1 && params.schoolType) {
        ...
        params.schoolType.forEach(async (type,index) => {
          const sql = "INSERT INTO school (type,name,startDate,endDate,status,transfer,major,score,scoreTotal) VALUES (:type,:name,:startDate,:endDate,:status,:transfer,:major,:score,:scoreTotal)";
     ...
     if (params.items && params.items.indexOf('교육이수') != -1) {
      ...
      if (params.items && params.items.indexOf('자격증') != -1) {
        if (!(params.licenseName instanceof Array)) {
          ...
          params.licenselssue = [params.licenselssue];
        ...
        params.licenseName.forEach(async(name,index) => {
          ...
          const replacements = {
            name : name,
            issue : params.licenselssue[index],
